                  ...
                  <v-flex xs12 sm8 md4>
                      <v-card class="elevation-12">
                          ...
                          <v-card-text>
                              ...
                              <v-alert type="success" v-model="exito" dismissible>Bienvenido al sistema: {{ nombres }}.</v-alert>
                              <v-form>
                                  <v-text-field v-model="user" :disable="inputDisable" v-on:keypress.enter="validar" name="user" prepend-icon="person" label="Usuario" type="text"></v-text-field>
    ...
    props: {
      ...
      methods:{
        login: function(){
         ...
         f.then(res => {
             let docRef = db.collection("usuarios").doc(this.user);
              if (res) {
                  this.exito = true;
                  docRef.get().then(doc =>{
                      this.nombres = doc.data()['nombres'] +' '+ doc.data()['apellidos'];
                  });
              }
             else
